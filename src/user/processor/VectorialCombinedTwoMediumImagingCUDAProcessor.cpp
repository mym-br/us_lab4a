...
// Must have the same values as in the .cu file.
						...
						Matrix<std::complex<MFloat>>& gridValue)
{
	...
	for (unsigned int i = 0; i < stepConfigList.size(); ++i) {
...
#ifdef USE_TRANSPOSE
		{
			...
			execTransposeKernel(gridDim, blockDim,
		...
		if (coherenceFactor_.enabled()) {
			...
			execProcessImagePCFKernel(procImageKernelGlobalSize / BLOCK_SIZE, BLOCK_SIZE,
#ifdef USE_TRANSPOSE
				data_->rawDataT.devPtr,
				rawDataN1_,
#else
			...
			execProcessImageKernel(procImageKernelGlobalSize / BLOCK_SIZE, BLOCK_SIZE,
#ifdef USE_TRANSPOSE
				data_->rawDataT.devPtr,
				rawDataN1_,
#else
