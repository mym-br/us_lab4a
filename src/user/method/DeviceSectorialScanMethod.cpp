...
DeviceSectorialScanMethod<TFloat>::getSingleImageFromNetwork()
{
	...
	std::vector<XYZ<float>> pointList = {{((config_.numElements - 1U) / 2.0f) * static_cast<float>(config_.pitch), 0.0, 0.0}};
...
template class DeviceSectorialScanMethod<float>;
template class DeviceSectorialScanMethod<double>;
