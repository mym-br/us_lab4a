...
CylinderDetectionAndFermatMethod<TFloat>::execCombinedTwoMediumImaging()
{
	...
	const auto firstRxElem        = methodPM->value<unsigned int>("first_rx_element"     ,           0, config.numElements - 1);
	const auto lastRxElem         = methodPM->value<unsigned int>("last_rx_element"      , firstRxElem, config.numElements - 1);
	...
	// Generate points in the interface.
	std::vector<XZ<TFloat>> interfacePointList(interfaceAngleList.size()); // x, z
...
//	bool coherenceFactorEnabled = coherenceFactor.enabled();
	VectorialCombinedTwoMediumImagingProcessor<TFloat> p(config, *acquisition, upsamplingFactor, coherenceFactor, maxFermatBlockSize, peakOffset);
	...
	p.process(stepConfigList, interfacePointList, rxApod, gridData);

	TFloat maxValue = 0;
	for (auto& item : gridData) {
		const TFloat absValue = std::abs(item.value);
		if (item.z >= normalizationMinZ && absValue > maxValue) {
			maxValue = absValue;
		}
	}
	...
	if (maxValue > 0) {
		...
		const TFloat normCoef = 1 / maxValue;
		for (auto& item : gridData) {
			if (std::abs(item.value) > maxValue) { // saturation
				item.value = 1;
			} else {
				item.value *= normCoef;
			}
...
//	if (coherenceFactorEnabled) {
//		LOG_DEBUG << "Saving the CF factors...";
//		project_.saveHDF5(gridData, imageDir + "/final_cf_factors", "factors", Util::CopyFactorOp());
...
//		LOG_DEBUG << "Saving the final image (after CF)...";
//		project_.saveHDF5(gridData, imageDir + "/final_cf_image", "image", Util::CopyValueOp());

//		project_.showFigure3D(2, "CF", &gridData, &interfacePointList,
...
template<typename TFloat>
void
CylinderDetectionAndFermatMethod<TFloat>::execCombinedTwoMediumImaging2()
{
	const ParamMapPtr methodPM = project_.getSubParamMap("method_config_file");
	const TwoMediumSTAConfiguration<TFloat> config(*project_.getSubParamMap("main_config_file"));
	const auto savedAcqDir        = methodPM->value<std::string>( "saved_acquisition_dir");
	...
	std::vector<typename VectorialCombinedTwoMediumImagingProcessor2<TFloat>::StepConfiguration> stepConfigList;
	unsigned int numActiveTxElements = 0;
	switch (project_.method()) {
	case MethodEnum::cylinder_detection_and_fermat_two_medium_imaging_combined_cyl_wave_2_dp:
	case MethodEnum::cylinder_detection_and_fermat_two_medium_imaging_combined_cyl_wave_2_sp:
		for (unsigned int i = 0; i < baseElemList.size(); ++i) {
			typename VectorialCombinedTwoMediumImagingProcessor2<TFloat>::StepConfiguration conf{
				i,
				baseElemList[i],
				txElem, txElem
			};
		...
		break;
	case MethodEnum::cylinder_detection_and_fermat_two_medium_imaging_combined_sta_2_dp:
	case MethodEnum::cylinder_detection_and_fermat_two_medium_imaging_combined_sta_2_sp:
		for (unsigned int i = 0; i < baseElemList.size(); ++i) {
			typename VectorialCombinedTwoMediumImagingProcessor2<TFloat>::StepConfiguration conf{
				i,
				baseElemList[i],
	...
	auto p = std::make_unique<VectorialCombinedTwoMediumImagingProcessor2<TFloat>>(
			config,
			acqDataList,
			upsamplingFactor,
			coherenceFactor,
