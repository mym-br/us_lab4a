...
from numpy.fft import fftn, ifft2
...
for tx_elem in range(NUM_ELEMENTS):
    ...
    tx_elem_signals = hdf5util.read_to_ndarray(file_path, SIGNAL_DATASET)
    signal_len = tx_elem_signals.shape[1]
    if NUM_ELEMENTS != tx_elem_signals.shape[0]:
    ...
    for rx_elem in range(NUM_ELEMENTS):
        signals_os[tx_elem, rx_elem, :signal_len] = tx_elem_signals[rx_elem, :]
...
#                    raise ValueError("k2.imag != 0.0")
                ...
                s_kvw_interp = \
                    (1.0 - ikv_factor) * ((1.0 - iw_factor) * s_kukvw[iku, ikv_base    , iw_base] + iw_factor * s_kukvw[iku, ikv_base    , iw_base + 1]) + \
                    ikv_factor         * ((1.0 - iw_factor) * s_kukvw[iku, ikv_base + 1, iw_base] + iw_factor * s_kukvw[iku, ikv_base + 1, iw_base + 1])
